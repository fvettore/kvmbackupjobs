CREATE TABLE `backup_days` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;CREATE TABLE `backup_log` (
...
CREATE VIEW `jobs_vms` AS select `v`.`vm` AS `vm`,`n`.`node` AS `node`,`j`.`name` AS `job_name`,`j`.`max_inc` AS `max_inc`,`bv`.`inc` AS `inc`,`bv`.`full` AS `full`,`j`.`enabled` AS `enabled`,`j`.`lastrun` AS `lastrun`,`j`.`lastcompletion` AS `lastcompletion`,`j`.`path` AS `path` from (((`backup_jobs` `j` join `backup_vms` `bv` on(`bv`.`idbackup_jobs` = `j`.`idbackup_jobs`)) join `vms` `v` on(`bv`.`idvms` = `v`.`idvms`)) join `nodes` `n` on(`n`.`idnodes` = `v`.`idnodes`)) order by `j`.`name`,`v`.`vm`;
